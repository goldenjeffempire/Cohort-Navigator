...
// ─── Learning Profiles (Adaptive Learning) ───────────────────────────────────
//
// One row per user — tracks their evolving competency model.
// Updated whenever they complete a challenge, quiz, or assignment.

export const aiLearningProfilesTable = pgTable("ai_learning_profiles", {
  id:            serial("id").primaryKey(),
  userId:        integer("user_id").notNull().unique().references(() => usersTable.id, { onDelete: "cascade" }),
  // Skill scores per topic area: { "javascript": 72, "arrays": 85, "oop": 60, ... }
  skillScores:   jsonb("skill_scores").$type<Record<string, number>>().notNull().default({}),
  // Topics with score < 60 (weak) and > 80 (strong)
  weakTopics:    jsonb("weak_topics").$type<string[]>().notNull().default([]),
  strongTopics:  jsonb("strong_topics").$type<string[]>().notNull().default([]),
  // Learning velocity: activities completed per week (EWM)
  learningVelocity: real("learning_velocity").notNull().default(0),
  // Risk level: none | low | medium | high
  riskLevel:     text("risk_level").notNull().default("none"),
  // Overall competency score 0-100 (mean of skill scores)
  competencyScore: real("competency_score").notNull().default(0),
  // Total AI messages sent by this student
  totalAIInteractions: integer("total_ai_interactions").notNull().default(0),
  // Most-used AI conversation mode
  preferredMode: text("preferred_mode"),
  // Personalised recommendations generated by adaptive engine
...
// ─── Skill Assessments ────────────────────────────────────────────────────────
//
// Individual assessment events that feed into ai_learning_profiles.
// One row per event (challenge submission, quiz attempt, AI evaluation).

export const aiSkillAssessmentsTable = pgTable("ai_skill_assessments", {
  id:         serial("id").primaryKey(),
  userId:     integer("user_id").notNull().references(() => usersTable.id, { onDelete: "cascade" }),
  skillArea:  text("skill_area").notNull(),   // e.g. "javascript", "arrays", "oop"
  score:      real("score").notNull(),         // 0-100
  source:     text("source").notNull(),        // "challenge" | "quiz" | "assignment" | "ai_eval"
  sourceId:   integer("source_id"),            // FK to the source table row
  evidence:   jsonb("evidence").$type<Record<string, unknown>>(),
  createdAt:  timestamp("created_at").notNull().defaultNow(),
}, (t) => ({
  userSkillIdx: index("ai_skill_user_idx").on(t.userId, t.skillArea),
  userDateIdx:  index("ai_skill_user_date_idx").on(t.userId, t.createdAt),
}));

// ─── Interview Sessions ───────────────────────────────────────────────────────
//
// Tracks a complete structured interview practice session.
// Stores Q&A pairs inline for session replay and report generation.

export const aiInterviewSessionsTable = pgTable("ai_interview_sessions", {
  id:           serial("id").primaryKey(),
  userId:       integer("user_id").notNull().references(() => usersTable.id, { onDelete: "cascade" }),
  sessionType:  text("session_type").notNull().default("technical"), // technical | behavioral | coding | mixed
  topic:        text("topic"),
  difficulty:   text("difficulty").notNull().default("medium"),
  // Array of { question, answer, score, feedback, keyPoints }
  ...
  userIdx: index("ai_interview_user_idx").on(t.userId),
}));

// ─── Content Generations ──────────────────────────────────────────────────────
//
// Tracks AI-generated learning content (quizzes, flashcards, summaries, etc.)
// so admins/mentors can review and reuse generated materials.

export const aiContentGenerationsTable = pgTable("ai_content_generations", {
  id:       serial("id").primaryKey(),
  userId:   integer("user_id").notNull().references(() => usersTable.id, { onDelete: "cascade" }),
  type:     text("type").notNull(),   // summary | flashcards | exercise | quiz | notes | challenge | capstone
  topic:    text("topic").notNull(),
  audience: text("audience").notNull().default("beginner"),
  content:  text("content").notNull(),
  metadata: jsonb("metadata").$type<Record<string, unknown>>(),
  rating:   integer("rating"),        // 1-5 user rating
  createdAt: timestamp("created_at").notNull().defaultNow(),
}, (t) => ({
  userIdx: index("ai_content_user_idx").on(t.userId),
  typeIdx: index("ai_content_type_idx").on(t.type),
}));
